/* for DEV/testing purposes, if you do not want to call API and use static data instead, uncomment at the end of this file following:
const senateData = senateDataSourceInput.results[0].members;
const houseData = houseDataSourceInput.results[0].members;
*/
...
let memberDataTables = ["error in JS"];
let atGlanceTables = ["error in JS"];
let leastMostTables = ["error in JS"];

async function fetchSenateDataFromAPI () {
  return (await fetch(
...
async function getSenateDataFromAPI () {
  try {
      data = await fetchSenateDataFromAPI();
...
async function fetchHouseDataFromAPI () {
  return (await fetch(
...
async function getHouseDataFromAPI () {
  try {
      data = await fetchHouseDataFromAPI();
      return data.results[0].members;
  } catch (error) {
      console.log("Error! -> " + error);
  }
...
const senateStatistics = {
  ...
  number_of_representatives_total: 0,
  voted_with_party_average_percentage_democracts: 0,
  voted_with_party_average_percentage_republicans: 0,
  voted_with_party_average_percentage_independents: 0,
  voted_with_party_average_percentage_total: 0,
  render_threshold: 0,
};
