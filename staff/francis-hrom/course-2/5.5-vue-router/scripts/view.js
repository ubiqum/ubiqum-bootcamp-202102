...
// send data to Vue instance in order to render "legislators" table data
async function renderLegislatorsTable(state) {
  app.members = await retrieveLegislators(state);
}

// parsing state parameter from URL

function setURLStateParam() {
  const selectedState = document.getElementById("stateFilterDropdown").value;
  const urlParams = new URLSearchParams(window.location.search);
  urlParams.set("state", selectedState);
...
async function setFirstItemInStateDropdown() {
  const statesList = await getStatesList();
  const stateAbbreviation = getURLStateParam();
  if (stateAbbreviation) {
    app.selectedState = statesList[stateAbbreviation];
  }
}

function getFullStateName(stateShortName) {
  return allStatesList.find((state) => state.shortName == stateShortName).name;
}
/*   for (const state of allStatesList) {
    if (state.shortName === shortStateName) {
      return state.name;
    }     */
